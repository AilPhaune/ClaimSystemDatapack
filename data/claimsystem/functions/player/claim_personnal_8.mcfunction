...
execute if entity @s[tag=NO_CLAIM_ADMIN_TOO_CLOSE] run tellraw @s {"text":"Vous êtes trop proche d'un claim admin","color":"red"}
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM,tag=SB_IS_ZERO,distance=..4] run tellraw @s {"text":"Vous êtes trop proche d'un de vos claims","color":"red"}
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM,tag=!SB_IS_ZERO,distance=..17] run tellraw @s {"text":"Vous êtes trop proche d'un claim de quelqu'un d'autre","color":"red"}
