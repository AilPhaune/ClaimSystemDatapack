...
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM,tag=SB_IS_ZERO,distance=..4] run tag @s add NO_CLAIM_MINE_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM16,tag=SB_IS_ZERO,distance=..6] run tag @s add NO_CLAIM_MINE_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM32,tag=SB_IS_ZERO,distance=..10] run tag @s add NO_CLAIM_MINE_TOO_CLOSE

# detect if too close to an enemy claim
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM,tag=!SB_IS_ZERO,distance=..17] run tag @s add NO_CLAIM_OTHER_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM16,tag=!SB_IS_ZERO,distance=..25] run tag @s add NO_CLAIM_OTHER_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM32,tag=!SB_IS_ZERO,distance=..41] run tag @s add NO_CLAIM_OTHER_TOO_CLOSE
