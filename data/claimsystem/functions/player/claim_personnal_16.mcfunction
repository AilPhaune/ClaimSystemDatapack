...
execute if entity @e[type=armor_stand,tag=CLAIM,distance=..25] run tag @s add NO_CLAIM_ADMIN_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=CLAIM16,distance=..33] run tag @s add NO_CLAIM_ADMIN_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=CLAIM32,distance=..49] run tag @s add NO_CLAIM_ADMIN_TOO_CLOSE

# detect if too close to another of the player's claims
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM,tag=SB_IS_ZERO,distance=..6] run tag @s add NO_CLAIM_MINE_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM16,tag=SB_IS_ZERO,distance=..8] run tag @s add NO_CLAIM_MINE_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM32,tag=SB_IS_ZERO,distance=..12] run tag @s add NO_CLAIM_MINE_TOO_CLOSE

# detect if too close to an enemy claim
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM,tag=!SB_IS_ZERO,distance=..25] run tag @s add NO_CLAIM_OTHER_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM16,tag=!SB_IS_ZERO,distance=..33] run tag @s add NO_CLAIM_OTHER_TOO_CLOSE
execute if entity @e[type=armor_stand,tag=PERSONNAL_CLAIM32,tag=!SB_IS_ZERO,distance=..49] run tag @s add NO_CLAIM_OTHER_TOO_CLOSE
...
execute unless entity @s[tag=NO_CLAIM_ADMIN_TOO_CLOSE] unless entity @s[tag=NO_CLAIM_MINE_TOO_CLOSE] unless entity @s[tag=NO_CLAIM_OTHER_TOO_CLOSE] run summon armor_stand ~ ~ ~ {Invulnerable:1b,NoBasePlate:1b,DisabledSlots:4144959,CustomName:'[{"text":"Personnal Claim","color":"gold"},{"text":" "},{"text":"[RADIUS = 16]","color":"aqua"}]',CustomNameVisible:1b,NoGravity:1b,PersistenceRequired:1b,Silent:1b,Tags:["PERSONNAL_CLAIM","PERSONNAL_CLAIM16","NEW_PERSONNAL"],display:{Name:'[{"text":"Claim [RADIUS = 16]","italic":false}]'}}
